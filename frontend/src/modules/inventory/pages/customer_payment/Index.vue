...
onMounted(() => {
    const toastMessage = sessionStorage.getItem('customerPaymentToastMessage')
    if (toastMessage) {
       const { message, type } = JSON.parse(toastMessage)
       toast.success(message)
    }
  sessionStorage.removeItem('customerPaymentToastMessage')
    ...
    <div class="card card-outline card-info">
        <div class="card-header">
            ...
            <div class="card-tools">
              <RouterLink class="btn btn-primary btn-sm" :to="`/inventory/customer-due-payment/create`"><i class="fas fa-plus"></i> Add New</RouterLink>
            </div>
        </div>
        <div class="card-body">
          ...
          <DataTable
           ref="dataTableRef"
                    :fields="[
                      ...
                      { key: 'payment_date', label: 'date', align: 'center',   isChange: true,},
                  ...
                  >
                    ...
                    <template #cell-payment_date="{ item }">
                      {{ userDateFormat(item.payment_date) }}
                    </template>
                   <template #actions="{ rowItem }">
                         <div class="btn-group dropleft">
                             <RouterLink :to="`customer-due-payment/${rowItem.id}`"  class="btn btn-sm btn-outline-primary"  >
