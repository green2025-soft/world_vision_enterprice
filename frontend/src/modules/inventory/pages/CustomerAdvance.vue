...
//  Setup
const title = 'Customer Advance'
const bUrl = 'inventory/customer-advance'
...
<template>
  ...
  size="lg"
    :button-disabled="customerBalance?.status === 'due'"
  
  @submit="saveItem"
>
  <ValidationErrors :errors="errors"  />
  <!-- Slot: Input fields -->
  <BaseFormGroup label="Customer Name" labelCols="3"  required>
     ...
     <ResourceSelect
              ...
              :isBranch="true"
            />
        <div v-if="customerBalance" class="text-muted mt-2">
      <span v-if="customerBalance.status === 'advance'" class="text-success">
        Advance: {{ customerBalance.total_balance }}
      </span>
      <span v-else class="text-danger">
        Due: {{ Math.abs(customerBalance.total_balance) }}
      </span>
    </div>
  </BaseFormGroup>
